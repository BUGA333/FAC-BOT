...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (!targetMember || !newRole) {
            return interaction.reply({ content: '❌ Erro ao processar rebaixamento. Dados não encontrados.', ephemeral: true });
        }
        ...
        targetMember.history.push({
            action: 'REBAIXAMENTO',
            details: `Rebaixado de ${oldRoleName} para ${newRole.name}. Motivo: ${reason}`,
        ...
        const embed = createEmbed(
            ...
            `<@${targetId}> foi rebaixado com sucesso para **${newRole.name}**!`,
            '#e67e22'
        );
        ...
        if (config && config.channels.logsRebaixamento) {
            const logChannel = interaction.guild.channels.cache.get(config.channels.logsRebaixamento);
            if (logChannel) {
