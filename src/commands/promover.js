...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption(option => 
            option.setName('membro')
                .setDescription('Membro a ser promovido')
                .setRequired(true))
    ...
    async execute(interaction, client) {
        ...
        const canPromote = await RPService.canPerformAction(interaction, target.id, 'PROMOTE');
        if (!canPromote) {
            return interaction.reply({ content: '❌ Você não tem permissão para promover este membro ou sua hierarquia é insuficiente.', ephemeral: true });
        }
        ...
        const availableRoles = await Role.find({ 
            ...
            level: { $gt: targetMember.currentRoleId ? targetMember.currentRoleId.level : -1 } 
        }).sort({ level: 1 });

        if (availableRoles.length === 0) {
            return interaction.reply({ content: '❌ Não há cargos superiores disponíveis para promoção.', ephemeral: true });
        }
        ...
        const row = new ActionRowBuilder()
            .addComponents(
                new StringSelectMenuBuilder()
                    .setCustomId(`promote_select_${target.id}_${encodeURIComponent(reason)}`)
